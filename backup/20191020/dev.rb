...
MAX_RESULTS=1000 # @todo put back to 100
DEBUG_MODE = false; # search for dev
...
def find_related_words(word)
  results_to_words(find_results("", word))
end
...
def find_results(rhyme, rel, print_header=false)
  if(rhyme.include?(" "))
    ...
    if(MAX_RESULTS != 100)
      request += "max=#{MAX_RESULTS}" # no trailing &, must be the last thing
    end
